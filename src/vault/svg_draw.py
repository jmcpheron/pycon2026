...
def draw_animated_comparison(filename: str | Path,
    ...
    Same SMIL pattern used elsewhere in this repo (one
    ``<animateTransform>`` per pin, one per cam). Renders inline in
    GitHub markdown via the camo proxy, just like
    ``gear-ratios-animated.svg``.
    """
    if counts is None:
